"""
Алгоритм Shield - покращений метод DCT для JPEG-зображень.
Працює тільки на ненульових квантованих коефіцієнтах (значення 0 та ±1
пропускаються). Кількість бітів, які можна сховати в одному коефіцієнті,
визначається його абсолютною величиною за таблицею ємності.
Порядок обходу блоків визначається ключем.
"""
...
KEY = 42
MESSAGE_SIZES = [100, 500, 2000, 5000, 10000]
...
AC_POSITIONS = [p for p in ZIGZAG if p != (0, 0)]


def capacity_bits(absval):
    """Таблиця 2.1 - залежність ємності від значення коефіцієнта."""
    if absval <= 1:
        return 0
    if absval <= 8:
        return 1
...
def embed(cover, message, key):
    ...
    for blk_idx in block_indices:
        ...
        for (u, v) in AC_POSITIONS:
            ...
            val = int(FQ[u, v])
            absval = abs(val)
            n = capacity_bits(absval)
            if n == 0:
                continue
            chunk = bits[bit_idx:bit_idx + n]
            if len(chunk) < n:
                chunk = chunk.ljust(n, '0')
            B = int(chunk, 2)
            new_abs = ((absval >> n) << n) | B
...
def main():
    ...
    for n in MESSAGE_SIZES:
        ...
        stego.save(f"{STEGO_DIR}/shield_{n}.png")
